...
class GameControllerTest {
    ...
    void getGameById_validId_game() throws Exception{
        ...
        MvcResult mvcResult = mockMvc.perform(get("/api/games/" + game2.getId())).andExpect(status().isOk()).andReturn();
        assertTrue(mvcResult.getResponse().getContentAsString().contains("Sushi"));
    ...
    void createGame_validGame_gameSaved() throws Exception{
        ...
        mockMvc.perform(put("/api/games/" + game3.getId()).content(body).contentType(MediaType.APPLICATION_JSON))
